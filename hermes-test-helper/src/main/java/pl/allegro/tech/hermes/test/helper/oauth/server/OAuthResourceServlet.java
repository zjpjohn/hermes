...
class OAuthResourceServlet extends HttpServlet {
    ...
    private void validateAccessToken(String username, String token) throws OAuthProblemException {
        if (!storage.accessTokenExists(username, token)) {
            throw OAuthProblemException.error("Invalid username or access token");
        }
    }
}
